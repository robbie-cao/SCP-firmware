...
static int atu_print_region(uint8_t region_idx, void *device_ctx_ptr)
{
    ...
    FWK_LOG_INFO("[ATU] Region %u", region_idx);
    ...
    if (region->region_size == 0) {
        FWK_LOG_INFO("[ATU] configured outside the firmware");
        return FWK_E_DATA;
    }
    ...
    FWK_LOG_INFO(
        "[ATU] [0x%" PRIX32 " - 0x%" PRIX32 "]-->[0x%" PRIX64 " - 0x%" PRIX64
        "]",
...
int get_available_region_index(void *device_ctx_ptr, uint8_t *region_idx)
{
    uint32_t atuc_reg_value;
    struct atu_device_ctx *device_ctx;
