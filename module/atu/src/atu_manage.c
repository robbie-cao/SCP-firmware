    ...
    uint8_t *region_idx)
{
    ...
    status = validate_atu_region_attributes(region->attributes);
    ...
    /*
     * Check if the translation region overlaps(logical address) any existing
     * translation regions mapped in the ATU.
     */
    ...
    status = get_available_region_index((void *)device_ctx, region_idx);
    if (status != FWK_SUCCESS) {
        return status;
    }
...
static int atu_device_init(fwk_id_t atu_device_id)
{
    ...
    for (uint8_t idx = 0; idx < device_ctx->config->atu_region_count; idx++) {
        ...
        FWK_LOG_ERR("[MOD_ATU] Initializing region %u", idx);

        /* Configure the translation region */
        status = atu_add_region(atu_region_config, atu_device_id, &region_idx);

        if (status != FWK_SUCCESS) {
            FWK_LOG_ERR("[MOD_ATU] Failed to initialize region %u", idx);
            return status;
        }
    }
    return FWK_SUCCESS;
...
void get_atu_manage_ops(const struct mod_atu_ops **atu_ops)
{
